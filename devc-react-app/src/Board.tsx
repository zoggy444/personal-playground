...
function Board({status, setStatus}) {
  ...
  const handleSlideRight = function () {
    ...
    for (let index in newValues) {
      newValues[index] = slideLine(newValues[index]);
    }
  ...
  const handleSlideLeft = function () {
    ...
    for (let i in newValues) {
      newValues[i] = slideLine(newValues[i], true);
    }
  ...
  const handleSlideDown = function () {
    ...
    for (let j = 0; j < 4; j++) {
      let newCol = slideLine(newValues.map((r) => r[j]));
      for (let i = 0; i < 4; i++) {
        newValues[i][j] = newCol[i];
      }
    }
  ...
  const endTurn = function (newValues) {
    const maxValue = Math.max(...[].concat(...newValues));
    if (status != 'won' && maxValue >= winCondition) setStatus('won');
    // a wild number appears...
    const numberAdded = addNewNumber(newValues);
